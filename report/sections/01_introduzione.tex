...
Il progetto \textbf{CCalibGA} si colloca in questo ambito e propone un
flusso di calibrazione che si distingue dall'approccio classico di
Zhang~\cite{Zhang2000} lungo due dimensioni:
...
\begin{enumerate}[label=\arabic*.]
  \item \textbf{Pattern planare custom.} Invece della scacchiera
        ...
        OpenCV~\cite{OpenCVccalib}, che localizza punti stabili
        sull'immagine di riferimento e li insegue nelle fotografie di
        calibrazione tramite matching di descrittori con verifica RANSAC.
...
Le Sezioni~\ref{sec:geom_proj}--\ref{sec:distorsione} presentano la
teoria matematica indispensabile, seguendo i capitoli 2, 6 e 7 di
Hartley e Zisserman~\cite{HZ2004}.
La Sezione~\ref{sec:pattern_custom} descrive il flusso geometrico del
matching con pattern custom a confronto con Zhang.
La Sezione~\ref{sec:ga} illustra il funzionamento dell'Algoritmo Genetico.
La Sezione~\ref{sec:risultati} riporta i risultati sperimentali.