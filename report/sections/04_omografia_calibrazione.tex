...
tramite l'equazione \(\homo{x}^T \mat{\Gamma}\, \homo{x} = 0\).
Una proiettività indotta da \(\mat{A}\) trasforma la conica secondo
\[
  \mat{\Gamma}' = (\mat{A}^{-1})^T \mat{\Gamma}\, \mat{A}^{-1}.
\]
La \emph{retroproiezione} di una conica immagine \(\mat{\Gamma}\)
nello spazio 3D definisce la conica
\(\mat{Q} = \mat{P}^T \mat{\Gamma}\, \mat{P}\).
Queste regole di trasformazione sono la base algebrica che permette
di propagare la geometria della conica assoluta dal piano all'infinito
fino al sensore della camera, come descritto nella sezione seguente.
...
\begin{oss}[Perché \(\mat{R}\) scompare]
La conica assoluta \(\Omega\) è \emph{invariante per rotazioni}: una
rotazione pura non sposta i punti circolari impropri nel piano
all'infinito, poiché è una trasformazione che conserva distanze e
angoli e agisce \emph{entro} lo spazio euclideo, senza toccare la
struttura del piano all'infinito. Di conseguenza, la conica immagine
\(\mat{\Gamma}\) che si forma sul sensore dipende \emph{solo} da
\(\mat{K}\): la rotazione \(\mat{R}\) che orienta la camera rispetto
alla scena non lascia traccia in \(\mat{\Gamma}\). Algebricamente
questo corrisponde al fatto che \(\mat{R}^T\mat{R} = \mat{I}_3\),
cancellando i termini rotazionali nel prodotto.
\end{oss}
...
\caption{Proiezione dei punti circolari impropri \(I=(1:+i:0)\) e
         ...
         la conica \(\mat{\Gamma}\) (IAC) sul sensore. Poiché \(\Omega\)
         è invariante per rotazioni, \(\mat{\Gamma}\) dipende solo da \(\mat{K}\).}
...
\[
  \vect{h}_1^T \mat{\Gamma}\, \vect{h}_2 = 0,
  \qquad
  \vect{h}_1^T \mat{\Gamma}\, \vect{h}_1
  = \vect{h}_2^T \mat{\Gamma}\, \vect{h}_2.
\]
